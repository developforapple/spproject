...
- (void)getBaseData:(void (^)(BOOL suc))completion
{
    if (self.baseDataFile) {
        ...
        ygweakify(self);
        [self.baseDataFile downloadWithOption:AVFileDownloadOptionIgnoringCachedData progress:^(NSInteger number) {
            ygstrongify(self);
            SPLog(@"基础数据：%d%%",(int)number);
            self.baseDataProgress = number/100.f;
            [self updateProgress];
        } completionHandler:^(NSURL *filePathURL, NSError *error) {
            ygstrongify(self);
            self.error = error;
            ...
            if (suc) {
                ...
                self.baseDataFilePath = filePath;
            }else{
...
- (void)getLangMainData:(void (^)(BOOL suc))completion
{
    if (self.langFile) {
        ...
        ygweakify(self);
        [self.langFile downloadWithOption:AVFileDownloadOptionIgnoringCachedData progress:^(NSInteger percentDone) {
            ygstrongify(self);
            SPLog(@"主语言文件：%d%%",(int)percentDone);
            self.langFileProgress = percentDone/100.f;
            [self updateProgress];
        } completionHandler:^(NSURL * _Nullable filePathURL, NSError * _Nullable error) {
            ygstrongify(self);
            self.error = error;
            NSString  *filePath = filePathURL.path;
            BOOL suc = [[NSFileManager defaultManager] fileExistsAtPath:filePath];
...
- (void)getLangPatchData:(void (^)(BOOL suc))completion
{
    if (self.langPatchFile) {
        ...
        ygweakify(self);
        [self.langPatchFile downloadWithOption:AVFileDownloadOptionIgnoringCachedData progress:^(NSInteger percentDone) {
            ygstrongify(self);
            ...
            if (suc) {
                SPLog(@"下载完成，路径：%@",filePath);
                self.langPatchFilePath = filePath;
            }else{
                SPLog(@"下载出错，文件不存在：%@",filePath);
            }
            completion?completion(error==nil && suc):0;
...
{
    if (self.baseDataFile && self.baseDataFilePath && [[NSFileManager defaultManager] fileExistsAtPath:self.baseDataFilePath]) {
        SPLog(@"解压缩基础数据");
        NSString *tmpFolder = [SPBaseData randomTmpFolder];
        NSString *zipPath = self.baseDataFilePath;
...
{
    if (self.langFile && self.langFilePath && [[NSFileManager defaultManager] fileExistsAtPath:self.langFilePath]) {
        SPLog(@"解压缩主语言文件");
        NSString *tmpFolder = [SPBaseData randomTmpFolder];
        NSString *zipPath = self.langFilePath;
        [SSZipArchive unzipFileAtPath:zipPath toDestination:tmpFolder overwrite:YES password:zipPassword progressHandler:^(NSString *entry, unz_file_info zipInfo, long entryNumber, long total) {
...
{
    if (self.langPatchFile && self.langPatchFilePath && [[NSFileManager defaultManager] fileExistsAtPath:self.langPatchFilePath]) {
        SPLog(@"解压缩语言补丁");
        NSString *tmpFolder = [SPBaseData randomTmpFolder];
        NSString *zipPath = self.langPatchFilePath;
        [SSZipArchive unzipFileAtPath:zipPath toDestination:tmpFolder overwrite:YES password:zipPassword progressHandler:^(NSString *entry, unz_file_info zipInfo, long entryNumber, long total) {
